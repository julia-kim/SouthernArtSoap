...
public class CartController {
	...
	public String cart(Model model) {
		...
		if (user != null) { // so cart doesn't crash if no one is logged in
			cartDetailses = (ArrayList) cartDetailsService
					.findAllCartDetailsesByCartIdSortedByDescendingDateCreated(user.getCart().getId());
		}
		...
		// Stripe checkout
		model.addAttribute("amount", 50 * 100); // in cents // what is this referring to? why is the amount set to
												// 5000???
	...
	// if (user == null)
	// return null;
	// List<CartDetails> userCartDetails =
	// cartService.findAllCartDetailsByUserId(user);
